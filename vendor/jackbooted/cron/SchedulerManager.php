...
class SchedulerManager extends WebPage {
    ...
    public function run( $default = '', $actionKey = self::ACTION ) {
        [ $clazz, $rest ] = explode( '->', Request::get( $actionKey, $default ) );
        [ $className, $functionName ] = self::normalizeCall( $clazz, $rest );
    ...
    public function history() {
        ...
        $cmdList = preg_split( '/\s+/', $schedItem->cmd, -1, PREG_SPLIT_NO_EMPTY );
        $tab = \Jackbooted\DB\DBTable::factory( \App\App::DB, $sql, [ $cmdList[0] ], \Jackbooted\DB\DB::FETCH_ASSOC );
