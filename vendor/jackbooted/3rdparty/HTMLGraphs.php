...
//----------------------------------------------------------------------------------------------------
    ...
    var $valuesHRef = '';                           // url link for the bar TODO BCD
    var $valuesTitle;                         // titles for the bar or for the link TODO BCD
    ...
    var $labelsTitle;                          // Titles for the link
    ...
    var $barLength = 2.0;                      // bar length ratio: float (from 0.1 to 2.9)
...
//----------------------------------------------------------------------------------------------------
    ...
    function build_value( $val, $max_dec, $sum = 0, $align = '' ) {
        ...
        if ( $sum )
            $sum = $this->number_format( $sum, $max_dec );
    ...
    function create_hBar( $value, $percent, $mPerc, $mPerc_neg, $max_neg, $mul, $valSpace, $bColor, $border, $spacer, $spacer_neg ) {
        ...
        if ( $percent < 0 ) {
            ...
            if ( $this->showValues < 2 )
                $bar .= '<span style="' . $this->cssPERCVALUES . '">' . $this->number_format( $percent, $this->percValuesDecimals ) . '%</span>';
            ...
            if ( $this->showValues < 2 )
                $bar .= '&nbsp;' . $this->number_format( $percent, $this->percValuesDecimals ) . '%';
            $bar .= '&nbsp;</td>';
        }
        $bar .= '</tr></table>';
    ...
    function create() {
        ...
        $valuesHRef = (is_array( $this->valuesHRef )) ? $this->valuesHRef : explode( ',', $this->valuesHRef );
        ...
        for ( $chart = $lcnt = 0; $chart < count( $val ); $chart++ ) {
            ...
            if ( $this->type == 'hbar' ) {
                ...
                for ( $i = 0; $i < count( $val[$chart] ); $i++, $lcnt++ ) {
                    ...
                    $graph .= '</tr></table></td>';
                    if ( $this->showValues < 2 )
                        $graph .= '<td style="' . $this->cssPERCVALUES . '" nowrap>&nbsp;' . $this->number_format( $percent, $this->percValuesDecimals ) . '%</td>';
                    $graph .= '</tr>';
                    if ( $this->labelSpace && $i < count( $val[$chart] ) - 1 )
